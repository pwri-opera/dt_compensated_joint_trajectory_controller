...
#include "dt_compensated_joint_trajectory_controller/dt_compensated_joint_trajectory_controller.hpp"
...
#include "dt_compensated_joint_trajectory_controller/trajectory.hpp"
...
{
DTCompensatedJointTrajectoryController::DTCompensatedJointTrajectoryController()
  : controller_interface::ControllerInterface(), dof_(0)
{
}

controller_interface::CallbackReturn DTCompensatedJointTrajectoryController::on_init()
{
  try
  {
...
DTCompensatedJointTrajectoryController::command_interface_configuration() const
{
...
controller_interface::InterfaceConfiguration
DTCompensatedJointTrajectoryController::state_interface_configuration() const
{
  controller_interface::InterfaceConfiguration conf;
  conf.type = controller_interface::interface_configuration_type::INDIVIDUAL;
...
void DTCompensatedJointTrajectoryController::read_state_from_hardware(JointTrajectoryPoint& state)
{
...
bool DTCompensatedJointTrajectoryController::read_state_from_command_interfaces(JointTrajectoryPoint& state)
{
...
bool DTCompensatedJointTrajectoryController::read_commands_from_command_interfaces(JointTrajectoryPoint& commands)
{
  bool has_values = true;
...
void DTCompensatedJointTrajectoryController::query_state_service(
    ...
    std::shared_ptr<control_msgs::srv::QueryTrajectoryState::Response> response)
{
  ...
  if ((traj_point_active_ptr_ && (*traj_point_active_ptr_)->has_trajectory_msg()))
  {
    ...
    response->success = (*traj_point_active_ptr_)
                            ->sample(static_cast<rclcpp::Time>(request->time), interpolation_method_, state_requested,
...
DTCompensatedJointTrajectoryController::on_configure(const rclcpp_lifecycle::State&)
{
  const auto logger = get_node()->get_logger();
  ...
  {
    RCLCPP_ERROR(logger,
                 "The DTCompensatedJointTrajectoryController does not support restarting with a different number of "
                 "DOF");
  ...
  joint_command_subscriber_ = get_node()->create_subscription<trajectory_msgs::msg::JointTrajectory>(
      ...
      std::bind(&DTCompensatedJointTrajectoryController::topic_callback, this, std::placeholders::_1));
  ...
  action_server_ = rclcpp_action::create_server<FollowJTrajAction>(
      ...
      std::bind(&DTCompensatedJointTrajectoryController::goal_received_callback, this, _1, _2),
      std::bind(&DTCompensatedJointTrajectoryController::goal_cancelled_callback, this, _1),
      std::bind(&DTCompensatedJointTrajectoryController::goal_accepted_callback, this, _1));
  ...
  query_state_srv_ = get_node()->create_service<control_msgs::srv::QueryTrajectoryState>(
      ...
      std::bind(&DTCompensatedJointTrajectoryController::query_state_service, this, _1, _2));

  return CallbackReturn::SUCCESS;
}

controller_interface::CallbackReturn DTCompensatedJointTrajectoryController::on_activate(const rclcpp_lifecycle::State&)
{
  ...
  for (const auto& interface : params_.command_interfaces)
  {
    ...
    {
      RCLCPP_ERROR(get_node()->get_logger(), "Expected %zu '%s' command interfaces, got %zu.", dof_, interface.c_str(),
  ...
  for (const auto& interface : params_.state_interfaces)
  {
    ...
    {
      RCLCPP_ERROR(get_node()->get_logger(), "Expected %zu '%s' state interfaces, got %zu.", dof_, interface.c_str(),
...
controller_interface::CallbackReturn
DTCompensatedJointTrajectoryController::on_deactivate(const rclcpp_lifecycle::State&)
{
...
controller_interface::CallbackReturn DTCompensatedJointTrajectoryController::on_cleanup(const rclcpp_lifecycle::State&)
{
  // go home
...
controller_interface::CallbackReturn DTCompensatedJointTrajectoryController::on_error(const rclcpp_lifecycle::State&)
{
  if (!reset())
  {
    return CallbackReturn::ERROR;
...
bool DTCompensatedJointTrajectoryController::reset()
{
  subscriber_is_active_ = false;
...
controller_interface::CallbackReturn DTCompensatedJointTrajectoryController::on_shutdown(const rclcpp_lifecycle::State&)
{
...
void DTCompensatedJointTrajectoryController::publish_state(const JointTrajectoryPoint& desired_state,
...
void DTCompensatedJointTrajectoryController::topic_callback(
    const std::shared_ptr<trajectory_msgs::msg::JointTrajectory> msg)
{
  ...
  // http://wiki.ros.org/dt_compensated_joint_trajectory_controller/UnderstandingTrajectoryReplacement
...
rclcpp_action::GoalResponse DTCompensatedJointTrajectoryController::goal_received_callback(
...
rclcpp_action::CancelResponse DTCompensatedJointTrajectoryController::goal_cancelled_callback(
...
void DTCompensatedJointTrajectoryController::goal_accepted_callback(
...
void DTCompensatedJointTrajectoryController::fill_partial_goal(
...
void DTCompensatedJointTrajectoryController::sort_to_local_joint_order(
...
bool DTCompensatedJointTrajectoryController::validate_trajectory_point_field(size_t joint_names_size,
                                                                             const std::vector<double>& vector_field,
                                                                             const std::string& string_for_vector_field,
                                                                             size_t i, bool allow_empty) const
{
...
bool DTCompensatedJointTrajectoryController::validate_trajectory_msg(
    const trajectory_msgs::msg::JointTrajectory& trajectory) const
{
  ...
  if (!params_.allow_nonzero_velocity_at_trajectory_end)
  {
    for (size_t i = 0; i < trajectory.points.back().velocities.size(); ++i)
    {
      ...
      {
        RCLCPP_ERROR(get_node()->get_logger(), "Velocity of last trajectory point of joint %s is not zero: %f",
  ...
  for (size_t i = 0; i < trajectory.points.size(); ++i)
  {
    ...
    if (params_.allow_integration_in_goal_trajectories)
    {
      const bool all_empty =
          points[i].positions.empty() && points[i].velocities.empty() && points[i].accelerations.empty();
...
void DTCompensatedJointTrajectoryController::preempt_active_goal()
{
  const auto active_goal = *rt_active_goal_.readFromNonRT();
  if (active_goal)
  {
...
void DTCompensatedJointTrajectoryController::set_hold_position()
{
...
void DTCompensatedJointTrajectoryController::resize_joint_trajectory_point(
...
void DTCompensatedJointTrajectoryController::resize_joint_trajectory_point_command(
    trajectory_msgs::msg::JointTrajectoryPoint& point, size_t size)
{
  if (has_position_command_interface_)
  {
...
PLUGINLIB_EXPORT_CLASS(dt_compensated_joint_trajectory_controller::DTCompensatedJointTrajectoryController,
